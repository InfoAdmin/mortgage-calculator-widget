...
Plugin URI: http://magnusharris.me
Description: A simple mortgage calculator widget
Version: 1.0.0
Author: Magnus Harris
Author URI: http://magnusharris.me
*/
...
function mc_mortgage_calc_css() {		
	wp_register_style( 'mc_mortgage_calc', plugins_url( './assets/style.css', __FILE__ ), false, '1.0' );
	wp_enqueue_style( 'mc_mortgage_calc' );
}
add_action( 'wp_print_styles', 'mc_mortgage_calc_css' );

/*-----------------------------------------------------------------------------------*/
/* Include JS */
/*-----------------------------------------------------------------------------------*/

function mc_mortgage_calc_scripts() {
	wp_register_script( 'calc', plugins_url( './assets/calc.js', __FILE__ ), array('jquery'), '1.0', true );
	wp_enqueue_script( 'calc' );
}
add_action( 'wp_enqueue_scripts', 'mc_mortgage_calc_scripts' );

/*-----------------------------------------------------------------------------------*/
/* Register Widget */
/*-----------------------------------------------------------------------------------*/

class mc_MortgageCalculator extends WP_Widget {

	function mc_MortgageCalculator() {
...
add_action( 'widgets_init', create_function( '', 'register_widget("mc_MortgageCalculator");' ) );
